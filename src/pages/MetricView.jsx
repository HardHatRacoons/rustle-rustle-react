...
function MetricView() {
  ...
  const grid = () => {
    ...
    pinned.forEach((key) => ordered.push({metric: metrics[key], idx: key}));
    for(let idx = 0; idx < metrics.length; idx++){
        if(!pinned.has(idx))
            ordered.push({metric: metrics[idx], idx});
    }
  ...
  return (
    <div className="select-none rounded-md border-solid border-2 border-sky-500 mx-2 mb-2 p-2 h-full bg-white">
        <div className="grid grid-cols-3 p-5 gap-4" aria-label="metrics">
            {
              grid().map((tile) =>
                <Card onChange={onPin} key={tile.idx} idx={tile.idx}>{tile.metric}</ Card>
              )
            }
        </div>
    </div>
