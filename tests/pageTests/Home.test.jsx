...
describe('Testing home page', () => {
    beforeAll(() => {
        vi.mock(import('react-router'), async (importOriginal) => {
            ...
            return {
                ...actual,
                useNavigate: vi.fn().mockReturnValue(vi.fn().mockImplementation((params) =>{
                        console.log(params);
                })),
            };
        });
        ...
        vi.mock('../../src/components/UserContext', () => ({
            useUser: vi.fn().mockReturnValueOnce({ sub: 'testuser' }).mockReturnValueOnce({ sub: 'testuser' }).mockReturnValueOnce(null).mockReturnValueOnce(null).mockReturnValue({ sub: 'testuser' }),
        }));
        ...
        vi.mock('aws-amplify/storage', async (importOriginal) => {
            ...
            return{
            ...actual,
            list: vi.fn().mockRejectedValueOnce(new Error("bucket not found"))
            ...
            getProperties: vi.fn().mockResolvedValueOnce({metadata: {}}).mockRejectedValueOnce(new Error("file not found"))
            .mockImplementation(async(path) => {
                  return {metadata: { name: path.path.split('/').at(-1) }}
              }),
              ...
              remove: vi.fn().mockRejectedValueOnce(new Error("file not found")).mockRejectedValueOnce(new Error("file not found"))
    ...
    test('file input and deletion testing', async() => {
        ...
        const fakeFile = new MockFile(['data'], 'example.pdf', { type: 'application/pdf', lastModified: new Date().getTime() });
